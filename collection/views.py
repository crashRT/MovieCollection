...
# @login_required
def index(requested):
    collections = collection.objects.order_by('id').reverse()
...
def taglist(requested, tagname):
    collections = collection.objects.filter(
        tags__name__in=[tagname]).distinct().reverse()
    taglist = Tag.objects.all()
    context = {
        'movie_list': collections,
        'taglist': taglist,
...
def creators(request):
    collections = collection.objects.order_by(
        'creator').values_list('creator', flat=True).distinct()
    taglist = Tag.objects.all()
    context = {
...
def creator_works(request, creator):
    works = collection.objects.filter(
        creator__icontains=creator).distinct().reverse()
    taglist = Tag.objects.all()
    context = {
        'movie_list': works,
        'taglist': taglist,
